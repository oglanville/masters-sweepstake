...
import { OWGR_PLAYERS } from "./owgr.js";
import { entriesLocked } from "./lockConfig.js";
...
/* ═══ DATA ═══ */
// Player DB is now built live from the current OWGR snapshot (src/owgr.js),
// keyed by slug. Picks reference these slugs, so every helper that reads P[key]
// keeps working unchanged.
const P = OWGR_PLAYERS;
...
/* ═══ 2026 U.S. OPEN PROJECTED PURSE — $21,500,000 total ═══
   Position payout table from the 2025 U.S. Open (Oakmont) breakdown. */
const US_OPEN_PURSE = {
  1: 4300000, 2: 2322000, 3: 1459284, 4: 1023014, 5: 852073,
  6: 755520, 7: 681131, 8: 610034, 9: 552103, 10: 507118,
  11: 462792, 12: 427901, 13: 398716, 14: 367995, 15: 341663,
  16: 319719, 17: 302164, 18: 284609, 19: 267054, 20: 249499,
  21: 234358, 22: 219217, 23: 204515, 24: 190910, 25: 179060,
  26: 168966, 27: 161286, 28: 154483, 29: 147900, 30: 141317,
  31: 134734, 32: 128151, 33: 121567, 34: 115643, 35: 110815,
  36: 105987, 37: 101379, 38: 96991, 39: 92602, 40: 88213,
  41: 83824, 42: 79436, 43: 75047, 44: 70658, 45: 66269,
  46: 62320, 47: 58370, 48: 54639, 49: 52445, 50: 50251,
  51: 48934, 52: 47837, 53: 46959, 54: 46520, 55: 46081,
  56: 45862, 57: 45642, 58: 45203, 59: 44765, 60: 44326,
};
...
function calcProjectedEarnings(players) {
  ...
  if (anyPos) {
    ...
    for (const [pos, group] of Object.entries(byPos)) {
      ...
      let total = 0;
      for (let i = 0; i < n; i++) total += US_OPEN_PURSE[posNum + i] || 0;
  ...
  while (i < ranked.length) {
    ...
    const posNum = i + 1;
    let total = 0;
    for (let k = 0; k < n; k++) total += US_OPEN_PURSE[posNum + k] || 0;
    const avg = Math.round(total / n);
...
// Entrants are loaded live from Supabase via GET /api/entries.
// Each row arrives as { name, picks:[{key,name,owgr,bucket,flag}], paid };
// useEntrants() maps it to the leaderboard's shape { name, picks:[key], paid }.
function useEntrants() {
  const [entrants, setEntrants] = useState([]);
  const load = useCallback(async () => {
    try {
      const r = await fetch("/api/entries");
      const d = await r.json();
      setEntrants((d.entries || []).map((e) => ({
        name: e.name,
        picks: (e.picks || []).map((p) => p.key),
        paid: !!e.paid,
      })));
    ...
    <div>
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        <div style={{flex:2,background:"#1a472a",borderRadius:10,padding:"12px 14px",textAlign:"center"}}>
          ...
          <div style={{fontSize:22,fontWeight:800,color:"#f4d35e",fontFamily:"'Playfair Display',serif"}}>£{ents.length * 10}</div>
    ...
    <div style={s.wrap}>
      <header style={s.hdr}>
        <div style={s.hdrIn}>
          ...
          <div style={{ flex: 1 }}>
            <h1 style={s.title}>The US Open Sweepstake <span style={{ fontWeight: 400, fontSize: "0.6em" }}>2026</span></h1>
            <div style={s.meta}>Shinnecock Hills · 18–21 June</div>
          </div>
          <button onClick={onBackToEntry} style={s.entryBtn}>{locked ? "My picks" : "✎ My entry"}</button>
        </div>
      ...
      <footer style={s.footer}>US Open Sweepstake · Live scores via ESPN, auto-refresh 60s · OWGR snapshot</footer>
    </div>
  );
}
...
const s = {
  ...
  entryBtn: { background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.25)", borderRadius: 8, color: "#f4d35e", fontSize: 11, fontWeight: 700, padding: "6px 10px", cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap", alignSelf: "flex-start" },
